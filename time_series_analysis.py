...
def app():
    ...
    st.text("""Autocorrelation as the name suggests is the degree of similarity between a given time series and a lagged version of itself over successive periods; in other words it measures the relationship between a variable’s value over the time.
...
In python we can easily plot the ACF by importing an inbuilt function “plot_acf” from “statsmodel.graphics.tsaplots.” We are attaching a graph where you can clearly notice the lag values where we suspect correlation to exist:
    ...
    st.text("As we observed in the previous section we learnt that ACF measures the linear relationship between an observation at time t and previous time periods. ")
